...
function agregarDetalle(idproducto, producto, precio_venta, uMedida, stock) {
    ...
    if (idproducto != "") {
        ...
        var fila = '<tr class="filas" id="fila' + cont + '">' +
            ...
            '<td><input type="hidden" name="stock[]" value="' + stock + '"><input type="hidden" name="idproducto[]"value="' + idproducto + '">' + producto + '</td>' +
...
function modificarSubototales() {
    ...
    for (var i = 0; i < cant.length; i++) {
        ...
        inpS.value = parseFloat(total - descuento);
        document.getElementsByName("subtotal")[i].innerHTML = inpS.value;

        if (parseFloat(inpSt.value) < parseFloat(inpC.value)){
            $("#btnG").hide();
